...
class ProgressAnalyticsSessionSyncManager(
    ...
    suspend fun syncProgressUpdate(progressId: Long, studentId: String): Boolean {
        return try {
            ...
            if (progress != null) {
                ...
                DebugLogger.debugLog(TAG, "⚠️ Progress not found or already synced: $progressId")
                false
            }
        } catch (e: Exception) {
    ...
    suspend fun syncAnalyticsUpdate(analyticsId: Long): Boolean {
        return try {
            ...
            if (analytics != null) {
                ...
                DebugLogger.debugLog(TAG, "⚠️ Analytics not found or already synced: $analyticsId")
                false
            }
        } catch (e: Exception) {
    ...
    suspend fun syncSessionUpdate(sessionId: String): Boolean {
        return try {
            DebugLogger.debugLog(TAG, "Real-time sync triggered for session: $sessionId")

            val allUnsyncedSessions = sessionDao.getUnsyncedSessions()
            val session = allUnsyncedSessions.find { it.sessionId == sessionId }
            ...
            if (session != null) {
                ...
                DebugLogger.debugLog(TAG, "✅ Session synced to Firestore: $sessionId, Duration: ${session.durationMillis}ms")
                true
            } else {
                DebugLogger.debugLog(TAG, "⚠️ Session not found or already synced: $sessionId")
                false
            }
        } catch (e: Exception) {
            DebugLogger.errorLog(TAG, "❌ Session real-time sync failed: ${e.message}")
            throw e
        }
    }
