    ...
    fun syncSessionUpdate(sessionId: String) {
        scope.launch {
            try {
                ...
                if (NetworkConnectivityObserver.isOnline(applicationContext)) {
                    DebugLogger.debugLog(TAG, " Real-time session sync starting: $sessionId")
                    try {
                        syncManager?.syncSessionUpdate(sessionId)
                        DebugLogger.debugLog(TAG, " Real-time session sync completed: $sessionId")
                    } catch (e: Exception) {
                        DebugLogger.errorLog(TAG, " Real-time session sync failed: ${e.message}")
                        scheduleBackgroundSync()
                    }
                } else {
                    DebugLogger.debugLog(TAG, " Device offline, session update queued: $sessionId")
                    scheduleBackgroundSync()
                }
            } catch (e: Exception) {
