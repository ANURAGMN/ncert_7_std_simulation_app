    ...
    suspend fun startSession() = withContext(Dispatchers.IO) {
        sessionMutex.withLock {
            try {
                ...
                // Get fresh student ID from SharedPreferences (IMPORTANT: Always get from shared prefs)
                val studentId = sharedPrefs.getUserId() ?: ""
                if (studentId.isBlank()) {
                    DebugLogger.debugLog("SessionManager", " StudentId is blank! User might not be logged in")
                }
            ...
            try {
                ...
                if (session != null) {
                    ...
                    DebugLogger.debugLog(
                        "SessionManager",
                        " Session ended: $sessionId, Duration: ${updatedSession.durationMillis / 1000}s, EndTime: ${endTime}"
                    )
    ...
    private suspend fun cleanupOldSession(oldSessionId: String) {
        try {
            ...
            if (oldSession?.sessionEndTime == null) {
                ...
                DebugLogger.debugLog("SessionManager", "Old session cleaned up, duration: ${updatedSession.durationMillis}ms")

                // Sync the old session AFTER updating with end time and duration
                DataSyncService.syncSessionUpdate(oldSessionId)
            }
        } catch (e: Exception) {
